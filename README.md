...
管理员在 **菜单设置** 中可将菜单类型设为「可配置报表」，填写 **SQL 模板**（仅允许单条 `SELECT` / `WITH…SELECT`，或 `EXEC` / `EXECUTE` 存储过程）与 **查询条件 JSON**。条件参数在 SQL 中必须使用 `**@参数名`**，且与 JSON 里每条 `name` 一一对应；执行时由服务端 **参数绑定**，不会把用户输入拼进 SQL 字符串。
