...
async function addMessage(pool, { conversationId, role, content, skillUsed, toolCalls }) {
  await pool
    .request()
    ...
    .input(
      ...
      toolCalls ? JSON.stringify(toolCalls).slice(0, 100000) : null
    )
...
async function getConversationMessages(pool, userCode, conversationId) {
  ...
  const rs = await pool
    ...
    .input('cid', sql.NVarChar(64), conversationId)
    .query(
      `SELECT id, role, content, skill_used, created_at
